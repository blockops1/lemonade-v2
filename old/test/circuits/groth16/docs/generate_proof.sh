...
# Configuration
CIRCUIT_NAME="lemonade_basic"
BUILD_DIR="build"
PHASE1_PATH="$BUILD_DIR/pot12_final.ptau"
POWERS_OF_TAU_URL="https://hermez.s3-eu-west-1.amazonaws.com/powersOfTau28_hez_final_16.ptau"
...
npx snarkjs groth16 setup "$BUILD_DIR/${CIRCUIT_NAME}.r1cs" "$PHASE1_PATH" "$BUILD_DIR/${CIRCUIT_NAME}_final.zkey"

echo -e "${GREEN}4. Contributing to phase 2 ceremony...${NC}"
echo "test" | npx snarkjs zkey contribute "$BUILD_DIR/${CIRCUIT_NAME}_final.zkey" "$BUILD_DIR/${CIRCUIT_NAME}_final_contributed.zkey"

echo -e "${GREEN}5. Exporting verification key...${NC}"
npx snarkjs zkey export verificationkey "$BUILD_DIR/${CIRCUIT_NAME}_final_contributed.zkey" "$BUILD_DIR/${CIRCUIT_NAME}_verification_key.json"
...
npx snarkjs groth16 prove "$BUILD_DIR/${CIRCUIT_NAME}_final_contributed.zkey" "$BUILD_DIR/witness.wtns" "$BUILD_DIR/proof.json" "$BUILD_DIR/public.json"
